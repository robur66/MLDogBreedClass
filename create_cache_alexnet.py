...
def prepare_mini_batch(k):
	...
	X_batch = np.asarray(X[k * mini_batch_size : (k + 1) * mini_batch_size].tolist(), dtype=np.float32) / 255
	return X_batch
...
# Run the initialization
	for k in range(num_complete_minibatches + 1):
		...
		fc6_result = sess.run(fc6, {X_t: X_batch})
		if k == 0: 
			my_array = fc6_result
		else:
			my_array = np.concatenate((my_array,fc6_result), axis=0)
